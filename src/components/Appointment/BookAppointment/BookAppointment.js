...
const BookAppointment = ({ date }) => {
    return (
        <section>
            <h2 className="text-primary text-center mb-5">Available Appointments on {date.toDateString()}</h2>
            <div className="row d-flex justify-content-center text-center">
                {
                    bookingData.map(bd => <BookingCard booking={bd} date={date}></BookingCard>)
                }
            </div>
        </section>
    );
